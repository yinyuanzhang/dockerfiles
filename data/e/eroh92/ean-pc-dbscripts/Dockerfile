...
# Sys::Hostname
# Data::Dumper
RUN apt-get update && apt-get install -y procps perl --no-install-recommends && rm -rf /var/lib/apt/lists/*
...
RUN DEBIAN_FRONTEND=noninteractive sh -c '( \
    apt-get update -q && \
    apt-get install -y -q zip wget && \
    apt-get clean && apt-get autoclean)' > /dev/null

RUN chown mysql:mysql /var/lib/mysql && touch /var/log/mysqld.log && chown mysql:mysql /var/log/mysqld.log
