...
LABEL maintainer="Mehedi Khan <mehedi.xpre@gmail.com>"
LABEL description="ElasticSearch 2.2.0"
...
ENV ROOT_PATH /usr/share/elasticsearch
WORKDIR $ROOT_PATH

ENV PATH /usr/share/elasticsearch/bin:$PATH

ENV ELASTICSEARCH_VERSION 2.2.0
ENV ELASTICSEARCH_TARBALL="https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-2.2.0.tar.gz" \
	ELASTICSEARCH_TARBALL_ASC="https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-2.2.0.tar.gz.asc" \
	ELASTICSEARCH_TARBALL_SHA1="4bd3ef681e70faefe3a66c6eb3419b5d4a0e2714"
...
# we shouldn't need much RAM to test --version (default is 2gb, which gets Jenkins in trouble sometimes)
	...
	if [ "${ELASTICSEARCH_VERSION%%.*}" -gt 1 ]; then \
		...
		${ROOT_PATH}/bin/plugin install mobz/elasticsearch-head; \
	else \
