FROM ruby:2.3.0
# Based on https://github.com/BBC-News/wraith/blob/master/Dockerfile
# some of ruby's build scripts are written in ruby
# we purge this later to make sure our final image uses what we just built
RUN apt-get update
RUN echo "export phantomjs=/usr/bin/phantomjs" > .bashrc
RUN apt-get install -y --force-yes libfreetype6 libfontconfig1 nodejs npm libnss3-dev libgconf-2-4
...
RUN apt-get update
RUN apt-get install -y --force-yes ttf-freefont ttf-mscorefonts-installer ttf-bitstream-vera ttf-dejavu ttf-liberation
