...
RUN tar xzvf /opt/spark-2.0.2-bin-hadoop2.7.tgz
RUN rm  /opt/spark-2.0.2-bin-hadoop2.7.tgz
...
RUN mv spark-2.0.2-bin-hadoop2.7 /opt/ && mkdir -p /user && mkdir -p /user/notebooks && mkdir -p /user/datasets

ADD entrypoint.sh /
ADD core-site.xml.datalake /opt/spark-2.0.2-bin-hadoop2.7/conf/
RUN chmod 777 /entrypoint.sh
ADD spark-defaults.conf /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.template
