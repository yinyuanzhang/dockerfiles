...
LABEL maintainer="Bogardo"

ENV DEBIAN_FRONTEND=noninteractive

RUN apt-get update && apt-get install -y gnupg tzdata software-properties-common

RUN add-apt-repository -y ppa:nginx/development \
    && add-apt-repository -y ppa:ondrej/php \
    && apt-get update \
    && apt-get install -y curl zip unzip git tmux supervisor sqlite3 \
