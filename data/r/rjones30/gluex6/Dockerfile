...
RUN yum -y install openmotif-devel libXpm-devel libXmu-devel libXp-devel
...
RUN yum -y install gsl gsl-devel libgnome-keyring lyx-fonts m4 neon pakchois mysql mysql-libs mysql-devel
...
RUN wget --no-check-certificate https://zeus.phys.uconn.edu/halld/gridwork/local6.tar.gz
