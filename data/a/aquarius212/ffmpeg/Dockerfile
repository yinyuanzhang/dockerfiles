...
RUN apt-get install -y -qq subversion make cmake git gcc-4.8 gfortran-4.8 cpp-4.8 g++-4.8 yasm autoconf automake nasm libtool vim augeas-tools augeas-lenses curl software-properties-common python-software-properties bison wget re2c lemon ruby

RUN add-apt-repository -y "deb http://us.archive.ubuntu.com/ubuntu/ trusty universe multiverse" && add-apt-repository -y "deb http://us.archive.ubuntu.com/ubuntu/ trusty-updates universe multiverse"
...
WORKDIR mplayer
RUN ./configure && make -j2 install && make clean
