...
RUN set -x && \
	...
	\
	CFLAGS="-g0 -O2" \
	CXXFLAGS="-g0 -O2 -DMAXTCPLISTENPORTS=${MAXTCPLISTENPORTS}" \
	LDFLAGS="-s" \
	\
	./configure \
		--build="$(uname -m)" \
...
CMD ["sh", "-c", "/usr/sbin/squid -f ${SQUID_CONFIG_FILE} -z && exec /usr/sbin/squid -f ${SQUID_CONFIG_FILE} -NCd 1"]
