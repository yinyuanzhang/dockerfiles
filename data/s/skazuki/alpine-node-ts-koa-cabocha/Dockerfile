...
CRFPP_FILE_ID=0B4y35FiV1wh7QVR6VXJ5dWExSTQ \
CRFPP_VERSION=0.58 \
...
\
# Installing cabocha
&& curl -sc ${APP_ROOT}/cookie "https://drive.google.com/uc?export=download&id=${CABOCHA_FILE_ID}" > /dev/null \
&& CODE="$(awk '/_warning_/ {print $NF}' ${APP_ROOT}/cookie)" \
&& curl -Lb ${APP_ROOT}/cookie "https://drive.google.com/uc?export=download&confirm=${CODE}&id=${CABOCHA_FILE_ID}" -o ${APP_ROOT}/cabocha-${CABOCHA_VERSION}.tar.bz2 \
&& cd ${APP_ROOT} && tar -jxf cabocha-${CABOCHA_VERSION}.tar.bz2 \
&& cd ${APP_ROOT}/cabocha-${CABOCHA_VERSION} \
