...
LABEL Description="Rebelizer's - nodejs container" Vendor="Rebelizer" Version="1.0"
...
RUN apk add --update --no-cache curl make gcc g++ python linux-headers paxctl libgcc libstdc++ gpgme   && \
  set -ex                                                                                              && \
  for key in \
...
RUN adduser -D ${RUNAS_USER}
USER ${RUNAS_USER}

ENV HOME /home/${RUNAS_USER}
WORKDIR ${HOME}
