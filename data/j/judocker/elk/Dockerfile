...
RUN \
    ...
    apt-get clean && \
    sed -i '/#cluster.name:.*/a cluster.name: docker.logstash' /etc/elasticsearch/elasticsearch.yml && \
    sed -i '/#discovery.zen.ping.multicast.enabled:.*/a discovery.zen.ping.multicast.enabled: false' /etc/elasticsearch/elasticsearch.yml && \
    sed -i '/#discovery.zen.ping.unicast.hosts:.*/a discovery.zen.ping.unicast.hosts: ["127.0.0.1"]' /etc/elasticsearch/elasticsearch.yml && \
