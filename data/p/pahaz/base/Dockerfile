...
RUN set -ex \
	...
	&& make install \
	&& ldconfig \
	&& pip3 install --no-cache-dir --upgrade --ignore-installed pip==$PYTHON_PIP_VERSION setuptools==$PYTHON_SETUPTOOLS \
	...
	&& cd /usr/local/bin && ln -s easy_install-3.5 easy_install3 \
	&& ln -s idle3 idle \
	&& ln -s pydoc3 pydoc \
	&& ln -s python3 python \
...
RUN npm install -g bower && npm install -g gulp && npm install -g yarn
